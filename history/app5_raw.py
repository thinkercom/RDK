...
# ====== 新增：中心区判定范围参数 ======
CENTER_X_RATIO = 0.4  # 左右中心区宽度占总宽度的百分比（如0.4表示40%）
...
def gen_frames():
    ...
    while True:
        ...
        if current_target:
            ...
            # y方向middle区间
            middle_upper = frame_height * 0.3  # 30%
            middle_lower = frame_height * 0.7  # 70%
            # 只有在左右中心区才判断上下方向
            if in_x_center:
        ...
        # ====== RECOVERING状态自动进入IDLE ======
        if device_state["system_state"] == DeviceState.RECOVERING:
            if not hasattr(gen_frames, "recover_start_time"):
                gen_frames.recover_start_time = time.time()
            elif time.time() - gen_frames.recover_start_time >= 3.0:
                update_device_state(0, 0, 0, DeviceState.IDLE)

                del gen_frames.recover_start_time
        else:
            if hasattr(gen_frames, "recover_start_time"):
                del gen_frames.recover_start_time
